...
def test_ai_call_invoked_with_filter_and_limit(monkeypatch):
    ...
    monkeypatch.setattr(
        pg_aiqo_report.litellm,
        "get_model_info",
        lambda model: {"max_input_tokens": 128000, "litellm_provider": "openai"},
    )
    monkeypatch.setattr(pg_aiqo_report.litellm, "token_counter", lambda **kwargs: 10)
    ...
    monkeypatch.setattr(pg_aiqo_report.litellm, "completion", fake_completion)
    monkeypatch.setattr(pg_aiqo_report.litellm, "completion_cost", lambda completion_response: 0.0)

    # Avoid filesystem output and heavy context loading during the test.
    monkeypatch.setattr(pg_aiqo_report.ReportGenerator, "generate_report", lambda *args, **kwargs: None)
    monkeypatch.setattr(pg_aiqo_report.ContextLoader, "load_all_contexts", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        pg_aiqo_report.ContextLoader,
        "build_full_prompt_with_optimizations",
        lambda self, **kwargs: "prompt",
    )
    monkeypatch.setattr(pg_aiqo_report.ContextLoader, "get_query_optimizations", lambda *args, **kwargs: None)

    # Force the query code to match the filter.
    monkeypatch.setattr(pg_aiqo_report.SQLUtils, "get_query_code", lambda *_: filter_code)

    args = pg_aiqo_report.parse_cli_arguments(
        [str(log_path), "-l", "1", "-f", filter_code, "-m", "gpt-4o"]
    )
    analyzer = pg_aiqo_report.PGAutoExplainAnalyzer(args)
    ...
    analyzer.context_loader.infra_context = None
